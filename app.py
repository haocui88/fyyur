...
def venues():
  ...
  all_areas = db.session.query(Venue.city, Venue.state).group_by(Venue.state, Venue.city).all()
  data = []

  for area in all_areas:
    area_venues = db.session.query(Venue.id, Venue.name).filter(Venue.state==area.staten, Venue.city==area.city).all()
    venue_data = []
...
def search_venues():
  ...
  search_result = db.session.query(Venue).filter(Venue.name.ilike('%{}%'.format(search_term))).all()
  data = []
...
def search_artists():
  ...
  search_term = request.form.get('search_term', '')
  search_result = db.session.query(Artist).filter(Artist.name.ilike('%{}%'.format(search_term))).all()
  data = []
...
def show_artist(artist_id):
  ...
  past_shows_query = db.session.query(Show).join(Venue).filter(Show.artist_id==artist_id).all()
  past_shows = []
  up_shows = []
